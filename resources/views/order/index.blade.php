...
@section('contents')
    ...
    <form action="{{ route('orders.index') }}" class="navbar-form navbar-left" method="get">
        <a href="{{ route('orders.index_day') }}" class="btn btn-primary" role="button">按日搜索</a>
        <a href="{{ route('orders.index_month') }}" class="btn btn-primary" role="button">按月份搜索</a>
        <div class="form-group">
            {{--<input type="month" class="form-control" name="keyword" >--}}
            <select name="keyword" class="form-control" id="">
